...
def mv_maxprob(
    ...
    prob_b = safe_softmax(logits_pred)
    return np.mean(pairwise_prob_diff(
        logits_gt=np.log(central_pred),
        logits_pred=np.log(prob_b)
    ).squeeze(), axis=0)
...
if __name__ == '__main__':
    ...
    for score, b_i, b_o, e_io, e_oo, e_oi, bi in [
        ...
        try:
            ...
            bayess_diff = b_i(A, B) - b_o(A, B)
            excess_oi = e_oi(A, B)
            bregman = bi(A, B)
            assert np.all(np.isclose(excess_diff, bayess_diff))
    ...
    for score, t_i, t_o, e_io, e_oo, e_oi, rbi, bias, mv in [
        ...
        try:
            ...
            assert np.all(np.isclose(e_oo(A, B), excess_oo))
            assert np.all(np.isclose(excess_io, decomposition_excess_io))
            print(f"{score} OK")
        except Exception as ex:
            print(f"{score} FAILED {ex}")
