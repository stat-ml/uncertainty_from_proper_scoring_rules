...
def aggregate_over_measures(
    ...
    measures = [
        c for c in dataframe_.columns if c not in ['OOD', 'InD', 'ScoringRule']
    ]
    res_dict_ = {val: [agg_func_] for val in measures}
    aggregated_ = dataframe_.groupby(by=by_).agg(
        res_dict_
    )
    return aggregated_
...
def collect_scores_into_dict_miss(
    ...
    scores_dict_ = defaultdict(
        list,
        {val: [] for val in dataframes_list_[0].RiskType.unique()}
    )
    ...
    for dataframe_ in dataframes_list_:
        for ind in dataframe_.training_dataset.unique():
            ...
            scores_dict_['ScoringRule'].append(
                df_aux_['LossFunction'].unique())
...
def collect_scores_into_dict(
    ...
    scores_dict_ = defaultdict(
        list,
        {val: [] for val in dataframes_list[0].RiskType.unique()}
    )
    std_dict_ = defaultdict(
        list,
        {val: [] for val in dataframes_list[0].RiskType.unique()}
...
def extract_same_different_dataframes(
    ...
    df = dataframe_.copy()
    df_logscore_logscore = df[(df['base_rule'] == 'Logscore') & (
        df['LossFunction'] == 'Logscore')]
    df_brier_brier = df[(df['base_rule'] == 'Brier') &
                        (df['LossFunction'] == 'Brier')]
    df_spherical_spherical = df[(df['base_rule'] == 'Spherical') & (
        df['LossFunction'] == 'Spherical')]

    df_logscore_not_logscore = df[(df['base_rule'] != 'Logscore') & (
        df['LossFunction'] == 'Logscore')]
    df_brier_not_brier = df[(df['base_rule'] != 'Brier')
                            & (df['LossFunction'] == 'Brier')]
    df_spherical_not_spherical = df[(df['base_rule'] != 'Spherical') & (
        df['LossFunction'] == 'Spherical')]

    dataframes_ = namedtuple('SameDiffDF', [
        'logscore_logscore', 'brier_brier', 'spherical_spherical',
        'logscore_not_logscore', 'brier_not_brier', 'spherical_not_spherical',
    ])
    return dataframes_(
