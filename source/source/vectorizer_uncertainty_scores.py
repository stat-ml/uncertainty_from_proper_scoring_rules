...
def pairwise_ce(logits_gt, logits_pred):
    ...
    ce_divs = np.sum(
        -gt_exp
        * (logits_pred_exp - logsumexp(logits_pred_exp, axis=-1, keepdims=True)),
        axis=-1,
    )
...
def bias_neglog(
    ...
    return pairwise_IS_distance(
        logits_gt=np.log(ppd), logits_pred=np.log(central_pred)
    ).squeeze()
...
def mv_neglog(
    ...
    mv_ = pairwise_IS_distance(
        logits_gt=np.log(central_pred), logits_pred=np.log(prob_b)
    ).squeeze()
...
def excess_neglog_outer_outer(
    ...
    return np.mean(pairwise_IS_distance(logits_gt, logits_pred), axis=(0, 1))
...
def excess_neglog_outer_inner(
    ...
    res = np.mean(
        pairwise_IS_distance(logits_gt=logits_gt, logits_pred=logits_ppd), axis=(0, 1)
    )
    return res
...
def excess_neglog_inner_outer(
    ...
    res = np.mean(pairwise_IS_distance(ppd_logits, logits_pred), axis=(0, 1))
    return res
...
def excess_neglog_inner_inner(
    ...
    res = np.mean(pairwise_IS_distance(ppd_logits_, ppd_logits), axis=(0, 1))
    return res
...
def excess_spherical_outer_outer(
    ...
    res = np.mean(pairwise_spherical(logits_gt, logits_pred), axis=(0, 1))
    return res
...
def excess_spherical_outer_inner(
    ...
    logits_ppd = np.log(posterior_predictive(logits_pred))
    res = np.mean(pairwise_spherical(logits_gt, logits_ppd), axis=(0, 1))
    return res
...
def excess_spherical_inner_outer(
    ...
    res = np.mean(pairwise_spherical(logits_ppd, logits_pred), axis=(0, 1))
    return res
...
def excess_spherical_inner_inner(
    ...
    logits_ppd = np.log(posterior_predictive(logits_pred))
    res = np.mean(pairwise_spherical(logits_ppd_, logits_ppd), axis=(0, 1))
    return res
...
def excess_maxprob_outer_outer(
    ...
    res = np.mean(
        pairwise_prob_diff(logits_gt=logits_gt, logits_pred=logits_pred), axis=(0, 1)
    )
    return res
...
def excess_maxprob_inner_outer(
    ...
    res = np.mean(
        pairwise_prob_diff(logits_gt=ppd_logits, logits_pred=logits_pred), axis=(0, 1)
    )
    return res
...
def excess_maxprob_outer_inner(
    ...
    res = np.mean(
        pairwise_prob_diff(logits_gt=logits_gt, logits_pred=ppd_logits), axis=(0, 1)
    )
    return res
...
def excess_maxprob_inner_inner(
    ...
    ppd_logits = np.log(posterior_predictive(logits_=logits_pred))
    res = np.mean(
        pairwise_prob_diff(logits_gt=ppd_logits_, logits_pred=ppd_logits), axis=(0, 1)
    )
    return res
...
def excess_brier_outer_outer(
    ...
    return np.mean(
        pairwise_brier(logits_gt=logits_gt, logits_pred=logits_pred), axis=(0, 1)
    )
...
def excess_brier_inner_outer(
    ...
    ppd_logits = np.log(posterior_predictive(logits_=logits_gt))
    return np.mean(
        pairwise_brier(logits_gt=ppd_logits, logits_pred=logits_pred), axis=(0, 1)
    )
...
def excess_brier_outer_inner(
    ...
    ppd_logits = np.log(posterior_predictive(logits_=logits_pred))
    return np.mean(
        pairwise_brier(logits_gt=logits_gt, logits_pred=ppd_logits), axis=(0, 1)
    )
...
def excess_brier_inner_inner(
    ...
    ppd_logits = np.log(posterior_predictive(logits_=logits_pred))
    return np.mean(
        pairwise_brier(logits_gt=ppd_logits_, logits_pred=ppd_logits), axis=(0, 1)
    )
...
def bias_logscore(
    ...
    return safe_kl_divergence(
        left_logits=np.log(ppd), right_logits=np.log(central_pred)
    ).squeeze()
...
def mv_logscore(
    ...
    return np.mean(
        safe_kl_divergence(
            left_logits=np.log(central_pred), right_logits=np.log(prob_b)
        ),
        axis=0,
    )
...
def total_logscore_outer(logits_gt: np.ndarray, logits_pred: np.ndarray) -> np.ndarray:
    ...
    """
    return np.mean(pairwise_ce(logits_gt, logits_pred), axis=(0, 1))
...
def excess_logscore_outer_outer(
    ...
    """
    return np.mean(pairwise_kl(logits_gt, logits_pred), axis=(0, 1))
...
def excess_logscore_inner_outer(logits_gt, logits_pred):
    ...
    """
    ppd_logits = np.log(posterior_predictive(logits_=logits_gt))
    return np.mean(pairwise_kl(ppd_logits, logits_pred), axis=(0, 1))
...
def excess_logscore_outer_inner(logits_gt, logits_pred):
    ...
    """
    ppd_logits = np.log(posterior_predictive(logits_=logits_pred))
    return np.mean(pairwise_kl(logits_gt, ppd_logits), axis=(0, 1))
...
def excess_logscore_inner_inner(logits_gt, logits_pred):
    ...
    ppd_logits_ = np.log(posterior_predictive(logits_=logits_gt))
    ppd_logits = np.log(posterior_predictive(logits_=logits_pred))
    return np.mean(pairwise_kl(ppd_logits_, ppd_logits), axis=(0, 1))
